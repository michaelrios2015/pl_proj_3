...
flight(2013,730,1000,lga,phx).
flight(2018,1030,1335,phx, lga).
...
connection(X, Y, Z, W) :- 
    %get departure time of first flight 
    flight(_, A, _, X, Y),
    %get landing time of second flight 
    flight(_, _, B, Z, W),
    %compare the two 
    (A > B -> true; false).
...
arrives(X):- 
    %gets all arriving time of any flight going to X
    flight(_, _, A, _, X),
    write("Flights to "), write(X), write(" arrive at "), write(A),nl.

%----------------------------------------------------------
%a bunch of helper functions for my path function
